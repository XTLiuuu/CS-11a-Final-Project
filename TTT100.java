...
public class TTT100 extends JFrame{ //lets TTT100 use JFrame elements
  ...
  */
  ...
  private ChangeBoardHandler bBHandler; //listens to actions performed on buttons boardB
  ...
  */
  public TTT100(){
    ...
    Container pane = getContentPane(); //new window
    ...
    for (int row = 0; row < 3; row++) {
      for (int column = 0; column < 3; column++){
        boardB[row][column] = new JButton(" "); //creates 9 new buttons corresponding to the 3x3 board array
        boardB[row][column].addActionListener(bBHandler); //creates the ActionListener that responds when button is pressed
        pane.add(boardB[row][column]); //puts button into window
      }
    }
